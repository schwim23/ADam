...
export const TTL = {
  DELIVERY_TODAY: 30 * 60,      // 30 min — GAM has ~1h data delay anyway
  DELIVERY_HISTORY: 6 * 3600,   // 6 hours — past dates don't change
} as const;
...
export class ReportCache {
  ...
  constructor(cacheDir?: string) {
    this.dir = cacheDir ?? join(process.env.HOME ?? homedir(), '.adam', 'cache');
  ...
  set<T>(key: string, data: T, ttlSeconds: number): void {
    ...
    const tmp = join(tmpdir(), `adam-${Date.now()}-${Math.random().toString(36).slice(2)}.tmp`);
