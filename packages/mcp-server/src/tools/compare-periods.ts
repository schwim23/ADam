import { z } from 'zod';
import type { DataClient, GamDimension } from '../data-client.js';
...
export async function handleComparePeriods(client: DataClient, args: z.infer<typeof comparePeriodsSchema>) {
  const dimensions: GamDimension[] = args.dimension ? [args.dimension as GamDimension] : [];
  ...
  if (!args.dimension) {
    // Network-wide totals comparison
